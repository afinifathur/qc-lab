# QC-DB - Safe Deployment Workflow
...
cd C:\laragon\www\qc-db\scripts
.\backup_qcdb_database.ps1
...
docker exec db mysqldump -u root -p[PASSWORD] qcdb > ~/backup_before_update_$(date +%Y%m%d_%H%M%S).sql
...
docker exec -i db mysql -u root -p[PASSWORD] qcdb < ~/backup_before_update_XXXXXXXX_XXXXXX.sql
...
cp /srv/docker/apps/qc-db/.env ~/env_backups/qc-db.env.backup

# Atau download ke Windows
scp peroniks@peroniks-ppicserver:/srv/docker/apps/qc-db/.env C:\laragon\www\qc-db\backups\.env.production
```
...
public function up()
{
    ...
    if (!Schema::hasColumn('tables_name', 'new_column')) {
        Schema::table('tables_name', function (Blueprint $table) {
...
php artisan make:migration add_new_column_to_tables_name_table
```
